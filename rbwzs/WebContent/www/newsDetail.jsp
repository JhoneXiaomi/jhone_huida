	...
	<div class="ej_main">
	  ...
	  <div class="ej_weizhi">
	     <div class="w1200 clearfix">
	       <div class="ej_weizhi_left fl"><span>N</span>DERECRATION &nbsp;说装修</div>
	       <div class="ej_local_right fr">您的当前位置：
	       <a href="${ctx }/">首页</a>&nbsp;>>&nbsp;
	       <a href="${ctx }/news/p1">说装修</a>&nbsp;>>&nbsp;
	       <c:if test="${news.type =='装修攻略' }">
	        <a href="${ctx }/domesticOutfit/p1">${news.type }</a>
	       </c:if>
	       <c:if test="${news.type =='家装日记' }">
	        <a href="${ctx }/decorateStrategy/p1">${news.type }</a>
	       </c:if>
	        <c:if test="${news.type =='行业资讯' }">
	        <a href="${ctx }/industry/p1">${news.type }</a>
	       </c:if>
	       </div>
	     </div> 
	  </div>
	        ...
	        <div class="tjanli">
	          ...
	          <c:forEach items="${aCasesList }" var="cases" begin="0" end="2">
		          <li>
			          ...
			          <span>
				           <c:choose>
				          		<c:when test="${fn:length(cases.caseTitle)>16 }">
				          				${fn:substring(cases.caseTitle,0,16) }
				          		</c:when>
