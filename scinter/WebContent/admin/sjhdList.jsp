	...
	<div id="js_center" data-options="region:'center'" class="noborder hidden">
		...
		<div id="toolbar" class="toolpanel datagrid-toolbar">
            ...
            <div class="rf reletive sch">
				<input class="txt" type="text" id="keyWord" name="keyWord" placeholder="请输入活动标题" />
			 		...
			 		<c:when test="${sessionScope.Role.roleName=='超级管理员' }">
	                  ...
	                  <select name="BCompany" id="BCompany" class="easyui-combobox" 
	            			...
	            			valueField: 'id',      
	             			...
	             			onLoadSuccess:function(data){}" style="width:247px;">
...
jQuery(function($){
	  ...
	  var $$=window.$$={
        ...
        loadGrid:function(param){
            $("#grid_recheck").datagrid({
                url:'${pageContext.request.contextPath}/admin/listMScActivatyToJson',
                fit:true,
                striped: true,
                pagination:true,
                pageSize: 10,
                ...
                columns:[[
					...
					{field:'title',align:'left',width:60,title:'标题'},
					{field:'person',align:'left',width:30,title:'创建人'},
					{field:'branchCompany',align:'left',width:30,title:'隶属分公司',
						formatter:function(value,row,index){
							return formatCompanyName(value);
						}
					},
					{field:'address',align:'left',width:70,title:'页面网址'},
                ...
                onSelect:function(index,row){
					...
					$("#framebottom").attr("src","${pageContext.request.contextPath}/admin/showMscActivaty?id="+row.id);
                },
    ...
    $("#js_edit").on("click",function(){
       ...
       if(roleIdArr.length==1){
    	   $("#framebottom").attr("src","${pageContext.request.contextPath}/admin/showMscActivaty?id="+roleIdArr[0]);
       }else{
    	   ...
    	   $.messager.confirm("确认", "确认删除选中的"+roleIdArr.length+"条活动?", function (r){  
	   	        if (r) {
	   	        	$.ajax({
	   		   			type : "POST",
	   		   			dataType : "json",
	   		   			url : "${pageContext.request.contextPath}/admin/delMscActivaty",
	   		   			data : "ids=" + roleIdArr,
	...
	$("#search").on("click",function(){
		...
		$("#grid_recheck").datagrid({url:'${pageContext.request.contextPath}/admin/searchMscListActivaty'}); //重新加载查询url
		$("#grid_recheck").datagrid('reload'); //重新加载
	});
