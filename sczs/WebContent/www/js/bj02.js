...
jQuery(function() {
    ...
    function getTotalDetailInfo(type) {
        ...
        data.onFirstStepEnd = function(data) {
            ...
            priceInDOM(data.priceInfo,jq('.budget-list li b'),data.res,data.banbaoPrice);
            //数据得到后样式变化
    ...
    //得到价格  并赋值到对应页面上去
    function priceInDOM(data,ele,homeMsg,banbao_price){
        var shi = 0;
        var ting = 0;
        var chu = 0;
        var wei = 0;
        ...
        jq('#quoted_other_bottom').html(data[homeMsg.length].price);
    }
