      ...
      <ul>
        <c:forEach items="${newsList }" var="news" varStatus="status">
        	<c:if test="${(status.index+1) % 2 != 0}" > 
			    ...
			    <li >
		          <div class="huodong_img"><a href="${ctx}/casesNewDetail?newsId=${news.newsId}&type=${news.type }"><img src="${news.thumbImg }" width="390" height="297" alt="${news.title }"/></a></div>
		          <div class="huodong_tit"><a href="${ctx}/casesNewDetail?newsId=${news.newsId}&type=${news.type }" style="color: #ff7200;">${news.title }</a></div>
		          ...
		          <div class="huodong_data">日期：<fmt:formatDate value="${news.updateTime }" pattern="yyyy-MM-dd HH:mm:ss"/>&nbsp;&nbsp;&nbsp;<p class="more fr"><a href="${ctx}/casesNewDetail?newsId=${news.newsId}&type=${news.type}" style="color: #fff;">活动详情</a></p></div>
		        </li>       	
        	</c:if>
			<c:if test="${(status.index+1) % 2 == 0}" > 
        	
			    <li class="no_mar">
		          <div class="huodong_img"><a href="${pageContext.request.contextPath}/casesNewDetail?newsId=${news.newsId}&type=${type }"><img src="${news.thumbImg }" width="390" height="297" alt="${news.title }"/></a></div>
		          <div class="huodong_tit"><a href="${pageContext.request.contextPath}/casesNewDetail?newsId=${news.newsId}&type=${type }" style="color: #ff7200;">${news.title }</a></div>
		          <div class="huodong_con"  style="height: 78;">
		          ...
		          <div class="huodong_data">日期：<fmt:formatDate value="${news.updateTime }" pattern="yyyy-MM-dd HH:mm:ss"/>&nbsp;&nbsp;&nbsp;<p class="more fr"><a href="${pageContext.request.contextPath}/casesNewDetail?newsId=${news.newsId}&type=${news.type}" style="color: #fff;">活动详情</a></p></div>
		        </li>         	
        	</c:if>        
      ...
      <div class="list_page">
       <div class="Page clearfix">
       	 ...
       	 <c:if test="${page.pageNo > 1}" >
         	<a href="${pageContext.request.contextPath}/spec/p${page.pageNo-1 }?type=${type }">上一页</a>     
         </c:if>
         <c:if test="${page.pageNo <= 1}" >
		 ...
		 <c:if test="${page.pageNo < page.pageCount}" >
         	<a href="${pageContext.request.contextPath}/spec/p${page.pageNo+1 }?type=${type }">下一页</a>
         </c:if>
         <c:if test="${page.pageNo >= page.pageCount}" >
         	<a href="javascript:void(0);">下一页</a>     
         </c:if>
         
         <a href="${pageContext.request.contextPath}/spec/p${page.pageCount }?type=${type }">最后一页</a> <span>${page.totalRecords }条记录 ${page.pageNo }/${page.pageCount } 页</span>
       </div>
    </div>
...
//	alert($(this).children('option:selected').val()); 
	...
	window.location.href="${pageContext.request.contextPath}/spec/p1?type=热门活动&BranchComp="+p1;
	});
	}); 
