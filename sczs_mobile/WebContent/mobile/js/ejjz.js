...
Zepto(function($){$('img[data-url]').imglazyload().on('error',function(e){e.preventDefault();});$("#header #siteBtn").click(function(){$(this).find("i").toggleClass("up");$("#header .city").toggleClass("show");});if($("#CateList").length>0){var bar=$('#CateList span.bar'),left=$('#CateList ul').offset().left;$('#CateList').navigator({select:function(e,index,li){bar.css({left:li.offsetLeft-left,width:li.childNodes[0].offsetWidth});},ready:function(){bar.appendTo($('#CateList').find('.ui-scroller'));}});}
if($("#listLoad").length>0&&$("#listLoadBox").length>0){if(currentpage<totalpage){$("#listLoad").click(function(){$("#listLoad i").removeClass("icon-angle-down").addClass("icon-spin5 animate-spin");$("#listLoad span").html('数据载入中...');$.get(window.location.pathname+'?pageid='+(currentpage+1)+'&ajax=1',function(data){$("#listLoadBox").append(data);currentpage=currentpage+1;$('img[data-url]').imglazyload().on('error',function(e){e.preventDefault();});$("#listLoad i").removeClass("icon-spin5 animate-spin").addClass("icon-angle-down");if(currentpage<totalpage){$("#listLoad span").html('点击加载更多');}else{$("#listLoad span").html('亲，没有了');}})});}else{$("#listLoad span").html('亲，没有了');}}
