...
<div id="owl-demo" class="owl-carousel">
	...
	<a class="item" href="" target="_blank"><img src="${ctx }/mobile/images/tcxw/banner02.jpg" alt=""><b></b></a>
</div>
	 ...
	 function submitAll(){
		...
		if(branchCompany=="0"){
			...
			$.ajax({
					...
					data : {'branchCompany':branchCompany,'orderId':orderId,'clientName':clientName,'phone':phone,'acreage':acreage,'sourceType':'头条'+source
						,'style':style},
					success : function(data) {
						if (data.data == "ok") {
		   				$('#allKaptchaImage').hide().attr('src', '${pageContext.request.contextPath}/getKaptchaImage?random=' + Math.floor(Math.random()*100) ).fadeIn();  
	  ...
	  if(branchCompany=="0"){
		 ...
		 alert("对不起，请正确填写电话格式!");
		  ...
		  $.ajax({
				...
				data : {'branchCompany':branchCompany,'orderId':orderId,'phone':phone,'sourceType':'头条'+source},
				success : function(data) {
					if (data.data == "ok") {
						$('#kaptchaImage').hide().attr('src', '${pageContext.request.contextPath}/getKaptchaImage?random=' + Math.floor(Math.random()*100) ).fadeIn();
						alert("请您稍等，稍后会有客服人员为您服务!");
